when HTTP_REQUEST {
    ...
    set debug 1
    if { $debug } {
        set logPrefix "[HTTP::host][HTTP::uri] | User-Agent: [HTTP::header value User-Agent] | Authorization: [HTTP::header value Authorization]"
    }
    ...
    switch -glob -- [string tolower [HTTP::header value User-Agent]] {
        ...
        default {
            ...
            if { $debug } { log local0.debug "Request from non-browser client: [HTTP::host][HTTP::uri] | User-Agent: [HTTP::header value User-Agent] | Authorization: [HTTP::header value Authorization] | Git-Protocol: [HTTP::header value Git-Protocol] | Headers: [HTTP::header names] | Cookies: [HTTP::header value Cookie]" }
    ...
    # IF we have an MRHSession Cookie:
    if { $debug } { log local0.debug "checking for mrhsession cookie..." }
    if { [HTTP::cookie names] contains "MRHSession" } {
        set sessionid [HTTP::cookie value "MRHSession"]
        if { $debug } { log local0.debug "Request with MRH Session Cookie: \'${sessionid}\'" }
        return
    }
...
when HTTP_RESPONSE_RELEASE {
    if { $debug } {
        log local0.debug "$logPrefix Server Response with [HTTP::status] | [HTTP::header names] | Content Length: [HTTP::header value "Content-Length"]"
    }
}
